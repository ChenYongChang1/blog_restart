...
const getUserAuth = async ($axios, store) => {
  const res = await $axios.post('http://120.55.96.72:7001/api2/user/login/', { name: 'cyc', password: 'Asd12345!' })
...
export default function({ app, $axios, store }) {
  ...
  $axios.onRequest(async (config) => {
    ...
    if (!store.state.assToken && config.url !== 'http://120.55.96.72:7001/api2/user/login/') {
